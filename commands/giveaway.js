...
module.exports.run = async (bot, message, args) => {
  ...
  var chanel = message.member.guild.channels.cache.find(n => n.name == "giveaways");
  //var give = message.guid.roles.cache.find(r => r.name == "giveaway");
  // Maak embed aan.
  ...
  var embedSend = await chanel.send(giveawayEmbed);
  embedSend.react("🎉");
  ...
  setTimeout(function () {
    ...
    for (var i = 0; i < winners.length; i++) {
      message.channel.send(`Gefeliciteerd: ${winners[y]} je hebt ${item} gewonnen in deze giveaway!  Maak een ticket aan binnen 24 uur om je cadeau te claimen!`);
    }
